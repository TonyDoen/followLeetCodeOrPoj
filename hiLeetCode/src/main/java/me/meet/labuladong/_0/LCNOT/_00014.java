...
public final class _00014 {
    private _00014() {
    }

    /**
     * LeetCode 「打家劫舍」系列问题（英文版叫 House Robber）怎么做，我发现这一系列题目的点赞非常之高，是比较有代表性和技巧性的动态规划题目，今天就来聊聊这道题目。
     * 打家劫舍系列总共有三道，难度设计非常合理，层层递进。第一道是比较标准的动态规划问题，而第二道融入了环形数组的条件，第三道更绝，让盗贼在二叉树上打劫，这就是传说中的高智商犯罪吧。。。
     * 下面，我们从第一道开始分析。
     * 
     *
     * House Robber I
     * 你是一个专业的盗贼,计划偷打劫街的房屋。每间房内都藏有一定宝的现金,影响你的唯一制约因素就是相邻的房屋装有相互连通的防盗系统,如果两间相邻的房屋在同一晚上被盗贼闯入,系统会自动报警。
     * 给定一个代表每个房屋存放金额的非负整数数组,计算你在不触动力警报装置的情况下,能够偷窃到的最高金额。
     *
     * 示例1:
     * 输入: [1,2,3,1]
     * 输出: 4
     ...
     * 如果你抢了这间房子，那么你肯定不能抢相邻的下一间房子了，只能从下下间房子开始做选择。如果你不抢这间房子，那么你可以走到下一间房子前，继续做选择。
     ...
     *
     * 你看他和我们的思路不一样，修改了递归函数的定义，略微修改了思路，使得逻辑自洽，依然得到了正确的答案，而且代码更漂亮。
     * 这就是我们前文 动态规划：不同的定义产生不同的解法 所说过的动态规划问题的一个特性。
    ...
    public int rob0(TreeNode root) {
        ...
        int do_it = root.getVal() +
            (root.getLeft() == null ? 0 : rob0(root.getLeft().getLeft()) + rob0(root.getLeft().getRight())) +
            (root.getRight() == null ? 0 : rob0(root.getRight().getLeft()) + rob0(root.getRight().getRight()));
        // 不抢，然后去下家
        int not_do = rob0(root.getLeft()) + rob0(root.getRight());
    ...
    int[] dp1(TreeNode root) {
        ...
        int[] left = dp1(root.getLeft());
        int[] right = dp1(root.getRight());
        // 抢，下家就不能抢了
        int rob = root.getVal() + left[0] + right[0];
        // 不抢，下家可抢可不抢，取决于收益大小
    ...
    // 返回 nums[start..] 能抢到的最大值
    private int dp3(int[] nums, int start) {
        if (start >= nums.length) {
            return 0;
        }
