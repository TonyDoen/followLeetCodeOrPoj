...
public final class _00013 {
     ...
     * }
     *
     * 这样，我们就可以不使用for循环也确切地知道每个二叉树节点的深度了。如果你能够理解上面这段代码，我们就可以来看 Dijkstra 算法的代码框架了。
     *
     *
     *
     * Dijkstra 算法框架
     ...
     * 第一题是力扣第 743 题「网络延迟时间」，难度 Medium，题目如下：
     * 有 n 个网络节点,标记为 1 到 n 。 给你一个列表times,表示信号经过有向边的传递时间。timmes[i]=(ui,vi,Wi),其中 ui 是源节点,vi是目标节点,Wi是一个信号从源节点传递到目标节点的时间。
     ...
     * 一开始你在最左上角的格子(0,0),且你你希望去最右下角的格子(rows-1,columns-1)(注意下标从0开始编号)。
     * 你每次可以往上,下,左,右四个方向之一移动,你想要找到耗费体力最小的一条路径。一条路径耗费的体力值是路径上相邻格子之间高度差绝对值的最大值决定的。
     ...
     * 最后看一道题吧，力扣第 1514 题「概率最大的路径」，难度 Medium，看下题目：
     * 给你一个由n个节点(下标从0开始)组成的无向加权图,该图由一个描述边的列表组成,其中 edges[i]=[a,b] 表示连接节点a和b的一条无向边,且该边遍历成功的概率为 succProb[i]
     * 指定两个节点分别作为起点start和终点end,请你找出从起点到终点成功概率最大的路径,并返回其成功概率。如果不存在从start到end的路径,请返回0。
     * 只要答案与标准答案的误差不超过1e-5,就会被视作正确答案。
     * 示例1:
     *             0
     *          /   |
     *    0.5  /    | 0.2
