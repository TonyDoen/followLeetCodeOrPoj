...
public final class LC0410 {
     ...
     * 还是注意观察 for 循环形式，只是不一定是func(i) == target作为终止条件，可能是<=或者>=的关系，这个可以根据具体的题目意思来推断，
     *
     *
     *
     *
     * 我们实操一下力扣第 410 题「分割数组的最大值」，难度 Hard：
     ...
     *
     *
     * 函数签名如下： int splitArray(int[] nums, int m);
     ...
     * 说个小插曲，快手面试有一道画师画画的算法题，很难，就是以这道题为原型。当时我没做过这道力扣题，面试有点懵，不过之前文章 二分查找算法运用
     * 写了两道类似的比较简单的题目，外加面试官的提示，把那道题做出来了。
     *
     ...
     * 但是回溯的缺点就是复杂度很高，我们刚才说的思路其实就是「组合」嘛，时间复杂度就是组合公式： C(m,n) = A(m,n)/m! = n!/(m!(n-m)!)
     ...
     *
     * 至此，这道题就通过二分查找技巧高效解决了。
     * 假设nums元素个数为N，元素和为S，则split函数的复杂度为O(N)，二分查找的复杂度为O(logS)，所以算法的总时间复杂度为O(N*logS)。
     */
    ...
    private static void testSplitArray() {
        int[] nums = new int[]{7,2,5,10,8};
        int m = 2;

        int rs = splitArray(nums, m);
