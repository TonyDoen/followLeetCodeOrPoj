...
public final class SimilarRgbColor {
    ...
    /**
     ...
     下面来看如何确定十位上的数字，比如拿"e1"来举例，其十进制数为225，其可能的选择有"ff"，"ee"，和"dd"，其十进制数分别为255，238，和221，我们目测很容易看出来是跟"dd"离得最近，但是怎么确定十位上的数字呢。我们发现"11"，"22"，"33"，"44"... 这些数字之间相差了一个"11"，十进制数为17，所以我们只要将原十六进制数除以一个"11"，就知道其能到达的位置，比如"e1"除以"11"，就只能到达"d"，那么十进制上就是"d"，至于个位数的处理情况跟上面一段讲解相同，我们对"11"取余，然后跟临界点"8"比较，如果个位上的数字大于"8"，那么十位上的数就加1。这样就可以确定正确的数字了，那么组成正确的十六进制字符串即可
     */
...
//    static String similarRGB(String src) {
//        if (null == src) {
//            return "";
//        }
//
//        int len = src.length();
//        char[] color = src.toCharArray();
//        for (int i = 1; i < len; i += 2) {
////            int num = stoi(color.substr(i, 2), nullptr, 16);
//            String sub = String.valueOf(color[i]) + color[i + 1];
//            int num = Integer.valueOf(sub, 16);
//            int idx = num / 17 + (num % 17 > 8 ? 1 : 0);
//            color[i] = (idx > 9) ? (char) (idx - 10 + 'a') : (char) (idx + '0');
//        }
